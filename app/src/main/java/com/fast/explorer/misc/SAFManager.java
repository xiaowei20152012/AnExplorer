...
import static com.fast.explorer.misc.IntentUtils.ACTION_OPEN_DOCUMENT_TREE;
import static com.fast.explorer.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return com.fast.explorer.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return com.fast.explorer.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return com.fast.explorer.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return com.fast.explorer.model.DocumentsContract.isTreeUri(uri);
    }
